...
class mld_pe_anomaly():
    ...
    def compute_MLD(self,Zc,dZ,Rho0_layer,dRho0dz_layer,energy,CNVG_T=1.e-5,Debug=False):
        ...
        PE_after[ACTIVE]      = 0.0
        PE_after_above          = np.add(np.NaN,np.array(np.zeros(ND)))
        PE_after_above[ACTIVE]  = 0.0
        ...
        while(z<NZ-1 and np.sum(ACTIVE)>0):
            ...
            if (np.sum(FINAL)>=1):
                ...
                PE_after_above[FINAL] = (
                    np.sum(PE_Kernel(Rho0_Mixed[:z,FINAL],
                                     Z_U[:z,FINAL],
                                     Z_L[:z,FINAL])
                           ,axis=0)
                )

                delta_PE_above = PE_after_above[FINAL]-PE_before_above[FINAL]
                delta_PE = PE_after[FINAL]-PE_before[FINAL]

                dZ_linear = (energy - delta_PE_above) / (delta_PE - delta_PE_above) * (Z_L[z,FINAL] - Z_L[z-1,FINAL])
                MLD[FINAL] = MLD[FINAL] - dZ[z,FINAL] + dZ_linear
