...
subroutine gls_stp( Hz      ,        &            ! Cell thickness           [m]
                    ...
                    count_condi,     &            ! for counting conditions of alpha_v_min and alpha_v_max
                    nstp    ,        &            ! time n
                    nnew    ,        &            ! time n+1
       ...
       integer,dimension( 0:9             ), intent(inout) :: count_condi
...
! local variables
       ...
       real(8)   :: alpha_n_min, alpha_m_max, alpha_v_min, alpha_v_max, cm0   , cm0inv2  , gls, du, dv
       real(8)   :: denom1, denom2, num1, num2
       real(8)   :: a_poly, b_poly, c_poly, delta_poly
       real(8)   :: tke_min_surf
       ...
       !--------------------------------------------------
          ...
          IF( ig == itke ) THEN
             ...
             ! ! NEW idea : TKE at the surface is at least equal to its value at N-1 (more precisely, to its tendency since N-2)
             ! ! Useful in free convection where the surface value is equal to 0 through the formula cm0inv2*ustar_sfc_sq
             ! tke_min_surf = MAX(  1.5*trb(N-1,nstp,ig)-0.5*trb(N-2,nstp,ig), tke_min )
             ! trb_sfc      = MAX( tke_min_surf, cm0inv2*ustar_sfc_sq )
             flux_top     = 0.
             ! bottom
...
!     &                       1.3*( 782.353/g )*ustar_sfc_sq(i,j)*(cff**1.5) )
              cff = 0.5*( trb(N-1,nnew,itke )+trb( N  ,nnew,itke ) )    ! NEW nnew => nstp
              !cff = trb(N-1,nnew,itke )    ! Correction ? (NEW nnew => nstp)
              ...
              ! bottom
              z0_b        = MAX( Zob , 1.E-04 )
              cff         = 0.5*( trb(1,nnew,itke ) + trb(0,nnew,itke ) )     ! NEW nnew => nstp
         ...
         DO k=1,N-1
            ...
            alpha_n(k)     = cff*     bvf(k)
            alpha_v(k)     = trb(k,nnew,itke) / epsilon**2 * (g * alpha)**2 * varT(k,nstp)
            !
