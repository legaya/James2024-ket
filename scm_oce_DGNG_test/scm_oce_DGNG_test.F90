...
contains
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  subroutine obl_stp_DGNG_test(  z_r  ,     &            ! Depth at cell centers    [m]
  ...
  ! local variables
         ...
         real(8),dimension( 0:N             )  :: shape_diff
         real(8), parameter ::  sigma_varT = 10.     ! K_varT = K_t / sigma_varT
         ...
         CASE DEFAULT
            call  gls_stp (Hz,u,v,varT,bvf,alpha,turb,lmix,eps,Akv,Akt,gamma_GLS,c_mu,c_mu_prime,c_mu_prime_star,        &
                           alpha_n,alpha_m,alpha_v,r_D,sustr,svstr,trb_scheme,sfunc_opt,dt,Zob,Neu_bot,kt,count_condi,        &
                           nstp,nnew,N,ntra,ngls,ntime)
  ...
  !------
            ...
            cff=-0.5*dt
            shape_diff = turb(:,nnew,1)**1.0
            shape_diff = shape_diff / MAXVAL(shape_diff)
            shape_diff = shape_diff - MINVAL(shape_diff)
            DO k=1,N-1
               !FC_2(k) = cff*(shape_diff * 4.0e-1 ) / (Hz(k)*sigma_varT)
         ...
         return
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  end subroutine obl_stp_DGNG_test
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
